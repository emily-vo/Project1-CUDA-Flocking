...
void Boids::initSimulation(int N) {
	...
	float cellWidthDivider = 1.0;
	gridCellWidth = (1.0 / cellWidthDivider) * 2.0f * std::max(std::max(rule1Distance, rule2Distance), rule3Distance);
...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf, const glm::vec3 *pos, const glm::vec3 *vel) {
	...
	for (int i = 0; i < N; i++) {
		if (i != iSelf) {
			...
			float distance = glm::length(thisPos - thatPos);
...
__global__ void kernComputeIndices(int N, int gridResolution,
	...
	glm::vec3 thisPos = pos[index];
	glm::vec3 cellPos = glm::floor((thisPos - gridMin) * inverseCellWidth);
	int cellIdx = gridIndex3Dto1D(cellPos[0], cellPos[1], cellPos[2], gridResolution);

	// TODO-2.1
	// - Label each boid with the index of its grid cell.
	gridIndices[index] = cellIdx;
...
__global__ void kernIdentifyCellStartEnd(int N, int *particleGridIndices,
	...
	int arrIdx = (blockIdx.x * blockDim.x) + threadIdx.x;
	if (arrIdx >= N) {
		return;
	}

	int gridIdx = particleGridIndices[arrIdx];
	int nextGridIdx = particleGridIndices[arrIdx + 1];

	if (arrIdx == N - 1) {
		gridCellEndIndices[gridIdx] = arrIdx;
		return;
	}
	if (arrIdx == 0) {
		gridCellStartIndices[gridIdx] = arrIdx;
	}
	if (gridIdx != nextGridIdx) {
		gridCellEndIndices[gridIdx] = arrIdx;
		gridCellStartIndices[nextGridIdx] = arrIdx + 1;
	}

}

__global__ void kernFlattenPosVel(int N, glm::vec3 *pos, glm::vec3 *vel1, glm::vec3 *coherent_pos, glm::vec3 *coherent_vel, int *particleArrayIndices) {
	int idx = (blockIdx.x * blockDim.x) + threadIdx.x;
	if (idx >= N) {
		return;
	}

	int particleIdx = particleArrayIndices[idx];
	coherent_pos[idx] = pos[particleIdx];
	coherent_vel[idx] = vel1[particleIdx];
}
...
__global__ void kernUpdateVelNeighborSearchScattered(
	...
	glm::vec3 gridIdx = glm::floor(gridPos);
	int flatIdx = gridIndex3Dto1D(gridIdx[0], gridIdx[1], gridIdx[2], gridResolution);

	glm::vec3 octant = glm::sign((thisPos - gridMin) - (gridIdx + glm::vec3(0.5)) * cellWidth);
	...
	for (int z = 0; z <= 1; z++) {
		for (int y = 0; y <= 1; y++) {
			for (int x = 0; x <= 1; x++) {
				glm::vec3 offset = glm::vec3(x, y, z) * octant;
				...
				int flatNeighborIdx = gridIndex3Dto1D(neighborIdx[0], neighborIdx[1], neighborIdx[2], gridResolution);
				// - For each cell, read the start/end indices in the boid pointer array.
				for (int i = gridCellStartIndices[flatNeighborIdx];
					i <= gridCellEndIndices[flatNeighborIdx]; i++) {
					if (i < 0) break;
					int boidIdx = particleArrayIndices[i];
					if (boidIdx != idx) {
						glm::vec3 thatPos = pos[boidIdx];
						glm::vec3 thatVel = vel1[boidIdx];
						float distance = glm::length(thisPos - thatPos);
						if (distance < rule1Distance) {
							rule1N++;
							center += thatPos;
	...
	newVel += thisVel + separate * rule2Scale;
...
void Boids::stepSimulationCoherentGrid(float dt) {
	...
	kernIdentifyCellStartEnd<<<fullBlocksPerGrid, blockSize>>>(numObjects, dev_particleGridIndices, dev_gridCellStartIndices,
		dev_gridCellEndIndices);
	//glm::vec3 *pos, glm::vec3 *vel1, glm::vec3 *coherent_pos, glm::vec3 *coherent_vel, int *particleArrayIndices
	kernFlattenPosVel << <fullBlocksPerGrid, blockSize >> > (numObjects, dev_pos, dev_vel1, coherent_pos, coherent_vel, dev_particleArrayIndices);
	kernUpdateVelNeighborSearchCoherent<<<fullBlocksPerGrid, blockSize>>>(numObjects, gridSideCount, gridMinimum, gridInverseCellWidth,
		gridCellWidth, dev_gridCellStartIndices, dev_gridCellEndIndices, coherent_pos, coherent_vel, dev_vel2);
